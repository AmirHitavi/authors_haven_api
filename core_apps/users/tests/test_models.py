...
def test_create_normal_user(normal_user):
    ...
    assert normal_user.pk_id is not None
    assert normal_user.date_joined is not None
...
def test_create_super_user(super_user):
    ...
    assert super_user.pk_id is not None
    assert super_user.date_joined is not None
...
def test_get_short_name(normal_user):
    ...
    expected_short_name = f"{normal_user.first_name.title()}"
...
def test_create_user_with_no_email(user_factory):
    ...
    assert str(error.value) == "Users must have an email."
...
def test_create_super_user_with_no_password(user_factory):
    ...
    assert str(error.value) == "Superusers must have password."


@pytest.mark.django_db
def test_create_super_user_without_staff(user_factory):
    with pytest.raises(ValueError) as error:
        user_factory.create(is_staff=False, is_superuser=True)

    assert str(error.value) == "Superusers must have 'is_staff' attribute set to True."


@pytest.mark.django_db
def test_create_super_user_without_active(user_factory):
    with pytest.raises(ValueError) as error:
        user_factory.create(is_staff=True, is_superuser=False)

    assert (
        str(error.value) == "Superusers must have 'is_superuser' attribute set to True."
    )
